...
tokens = [
    ...
    "PROTOCOLO", "PUERTO", "URL", "INTEGER","EMAILT"
]
...
def t_URL(t):
    r'"(?:http|https)://(?:(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)\.)+(?:[A-Za-z]{2,})(?::[0-9]+)?(?:/[A-Za-z0-9\-_.~/#:?@!$&\'()*+,;=%]*)?"\s*'
    raw_url = t.value[1:-1]
    # 1) Control de ".." en dominio (como antes)
    if '..' in raw_url:
        raise SyntaxError(f"URL inválida en línea {t.lineno}: dominio con '..'")
    # 2) Control de "//" en ruta (excluyendo el "://" inicial)
    #    buscamos dos o más barras seguidas que no formen parte de "://"
    if re.search(r'(?<!:)/{2,}', raw_url):
        raise SyntaxError(f"URL inválida en línea {t.lineno}: ruta con '//' → {raw_url}")
    t.value = raw_url
    return t
...
def t_EMAILT(t):
    ...
    # Extraemos el valor sin las comillas
    raw = t.value[1:-1]
    # (Opcional) rechazamos dobles puntos: usuario..dom
    if '..' in raw:
        raise SyntaxError(f"Email inválido en línea {t.lineno}: contiene '..' → {raw}")
    t.value = raw
    return t
